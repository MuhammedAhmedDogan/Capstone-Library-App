        ...
        {books.map((item, index) => (
          <div key={item.id} className='item-card'>
            <div className='book-informations'>
              ...
              <h2 style={index % 2 === 0 ? { backgroundColor: '#E5E4E2' } : {}}>{item.publicationYear}</h2>
              <h2 style={index % 2 === 0 ? { backgroundColor: '#E5E4E2' } : {}}>{item.author.name}</h2>
              <h2 style={index % 2 === 0 ? { backgroundColor: '#E5E4E2' } : {}}>{item.publisher.name}</h2>
              <h2 style={index % 2 === 0 ? { backgroundColor: '#E5E4E2' } : {}}>{item.categories.map((category, index) => (
                <span key={category.id}>
                  {category.name}{index < item.categories.length - 1 ? ', ' : ''}
                </span>))}</h2>
              <h2 style={index % 2 === 0 ? { textAlign: 'center', backgroundColor: '#E5E4E2', width: '80px' } : { textAlign: 'center', width: '80px' }}>{item.stock}</h2>
            </div>
